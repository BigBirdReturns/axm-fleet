...
```bash
./demo.sh          # beat 4 needs a Go toolchain and an axm-genesis checkout:
                   # AXM_GENESIS=/path/to/axm-genesis ./demo.sh
                   # (defaults to a sibling ../axm-genesis directory)
```
...
builds, and cross-verification by the independent Go verifier. Requirements are defined in the kernel's
[`spec/v1/CONFORMANCE.md`](https://github.com/BigBirdReturns/axm-genesis/blob/main/spec/v1/CONFORMANCE.md).

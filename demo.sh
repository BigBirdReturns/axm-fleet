...
#
# Requires: pip install 'axm-genesis[mldsa-compat]' (see README) && pip install -e . ;
# beat 4 additionally requires a Go toolchain and a checkout of axm-genesis
# for its verifiers/go — set AXM_GENESIS, or keep it as a sibling directory.
#
...
AXM_GENESIS="${AXM_GENESIS:-$HERE/../axm-genesis}"
WORK="$(mktemp -d)"
...
if command -v go >/dev/null 2>&1 && [ -d "$AXM_GENESIS/verifiers/go" ]; then
  (cd "$AXM_GENESIS/verifiers/go" && go build -o "$WORK/axm-verify-go" ./cmd/axm-verify-go)
  "$WORK/axm-verify-go" shard "$POOL/patch" --trusted-key "$PUB"
  echo "(second implementation, built from the spec alone: PASS)"
else
  echo "(skipped: need a Go toolchain and AXM_GENESIS pointing at an axm-genesis checkout)"
fi

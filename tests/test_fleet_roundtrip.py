...
def test_independent_go_verifier_accepts(tmp_path, keypair):
    ...
    candidates = [
        Path(os.environ["AXM_GENESIS"]) if os.environ.get("AXM_GENESIS") else None,
        SPOKE_ROOT.parent / "axm-genesis",   # sibling checkout
        SPOKE_ROOT.parents[1],               # in-kernel-repo layout (spokes/axm-fleet)
    ]
    go_dir = next(
        (c / "verifiers" / "go" for c in candidates
         if c and (c / "verifiers" / "go").is_dir()),
        None,
    )
    if go_dir is None:
        pytest.skip("no axm-genesis checkout found (set AXM_GENESIS for verifiers/go)")
